...
import { handleTimeText, updatePlaylistsValue, updateSongsListValue } from "../../utils/appHelpers";
import {
...
export default function SongListItem({
      ...
      try {
         ...
         if (data.in_playlist.length && userPlaylists?.length) {
            ...
            for (let playlistId of data.in_playlist) {
               const index = userPlaylists.findIndex((playlist) => playlist.id === playlistId);
   ...
   const handleAddSongToPlaylist = async (song: Song, playlist: Playlist) => {
      ...
      const newSong: Song = { ...song, in_playlist: [...song.in_playlist, playlist.id] };
   ...
   const songComponent = useMemo(
    ...
    songContainers-center justify-center items-center hidden group-hover/image:flex"
      ...
      [isCheckedSong, isSelected, inProcess, active]
   );
