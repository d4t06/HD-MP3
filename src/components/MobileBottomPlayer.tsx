...
const MobileBottomPlayer: FC<Props> = ({
  ...
  const handlePlayPause = () => {
    playStatus.isPlaying ? pause() : play();
  };
  ...
  const renderIcon = useMemo(() => {
    if (playStatus.isWaiting) {
      return <ArrowPathIcon className={"w-[36px] animate-spin"} />;
    } else if (playStatus.isError && songInStore.name) {
  ...
  }, [playStatus.isWaiting, playStatus.isError, playStatus.isPlaying]);
            ...
            <div className="flex-grow  ml-[10px]">
              {songInStore.song_url && (
                <>
                  ...
                  <p className={`text-[14px] font-[400] opacity-60 line-clamp-1`}>
