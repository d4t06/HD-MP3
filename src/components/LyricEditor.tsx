...
export default function LyricEditor({
  ...
  const { userSongs, setUserSongs, adminSongs, setAdminSongs } =
    useSongsStore();
  ...
  useEffect(() => {
    ...
    if (baseLyricArr.length && firstTimeRender.current)
  ...
  const ctaComponent = (
    <div className={`flex justify-between gap-[10px] mb-[20px] ${text}`}>
      <div className="left flex flex-wrap gap-[10px]">
        <button
          onClick={() => handleOpenModal("add_base_lyric")}
        ...
        {isPlaying ? (
          ...
          <Button
            ...
            onClick={play}
            className={
              style.button + (isPlaying || !isCanPlay ? style.disable : "")
            }
          >
            Play
          </Button>
        ...
        <Button
          ...
          className={
            style.button +
            (!realTimeLyrics.length || !baseLyricArr.length
              ? style.disable
              : "")
          }
        >
  ...
  return (
    <div className={`${loading ? "opacity-[0.6] pointer-events-none" : ""}`}>
      ...
      <div
        className={`${style.row} ${style.section} mb-[0] h-[55vh] pb min-h-[200px] overflow-auto`}
      >
        <div className={style.lyricBox + "min-w-[50%] max-w-[50%]"}>
          {!!baseLyricArr.length ? (
            <>
          ...
          {modalComponent.current === "turtorial" && (
            <>
              ...
              <div className="h-[500px] max-h-[75vh] overflow-y-auto no-scrollbar">
                ...
                <h1 className="font-semibold mb-[12px]">
                  Cách thêm lời bài hát:
                </h1>
                <img
                  className="w-full border rounded-[16px]"
                  src={tutorial1}
