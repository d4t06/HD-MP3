...
import { ChevronUpIcon, QueueListIcon, SpeakerWaveIcon, SpeakerXMarkIcon } from "@heroicons/react/24/outline";
...
export default function BottomPlayer({
   ...
   const { handleSetVolume, isMute, handleMute } = useVolume(volumeLineWidth, volumeProcessLine, audioEle);
   ...
   const classes = {
      ...
      wrapper: `border-${theme.alpha} fixed bottom-0 w-full border-t transition-transform z-50 px-10 ${
         admin ? "h-[60px]" : "h-[90px]"
      }`,
      ...
      volumeWrapper: `volume-control ${admin ? "w-1/4" : "w-1/4 "} flex items-center justify-end gap-[8px]`,
   ...
   return (
      ...
      >
         <div
            className={`${classes.blurBg} ${isOpenFullScreen ? "opacity-0 transition-opacity delay-[.2s]" : ""}`}
         ></div>
         <div
         ...
         >
            ...
            {/* control */}
            <div
               className={` ${classes.controlWrapper} ${!admin ? classes.controlWrapperChild_1 : ""}  ${
                  classes.controlWrapperChild_2
               }`}
            >
               {useMemo(
                  () => (
                     <Control admin={admin} audioEle={audioEle} idle={false} isOpenFullScreen={isOpenFullScreen} />
                  ),
            ...
            <div className={`${classes.volumeWrapper}  ${isOpenFullScreen ? "hidden" : ""}`}>
               <div className="flex flex-grow items-center max-w-[150px]">
                  <button onClick={() => handleMute()}>
                     {isMute ? <SpeakerXMarkIcon className="w-6 h-6" /> : <SpeakerWaveIcon className="w-6 h-6" />}
                  </button>
                  <div
                  ...
                  >
                     <div ref={volumeProcessLine} className={`${classes.volumeLineCurrent} ${theme.content_bg}`}></div>
                  </div>
               </div>
