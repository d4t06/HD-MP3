...
function FullScreenPlayer({ isOpenFullScreen, setIsOpenFullScreen, idle, audioEle }: Props) {
  ...
  // methods
  const handleClickNext = useDebounce(() => handleScroll("next"), 300);
  const handleClickPrevious = useDebounce(() => handleScroll("previous"), 300);
          ...
          <div className={classes.header}>
            ...
            {idle && (
              <div className={`${classes.headerCta} left-0`}>
                <img className={`${idle ? "h-full" : "h-full"} mr-[10px]`} src={zingIcon} alt="" />
                {activeTab === "Lyric" && (
