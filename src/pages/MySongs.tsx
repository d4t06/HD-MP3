import { PlusCircleIcon, QueueListIcon, StopIcon, TrashIcon, XMarkIcon } from "@heroicons/react/24/outline";
...
export default function MySongsPage() {
   ...
   const handleGetFavorite = async () => {
      ...
      try {
         const queryGetFavorites = query(collection(db, "songs"), where("id", "in", userInfo.like_song_ids));
         ...
         if (favoritesSnap.docs.length) {
            const favorites = favoritesSnap.docs.map((s) => ({ ...s.data(), song_in: "favorite" } as SongWithSongIn));
   ...
   const handleSetFavoriteSong = (song: Song, index: number) => {
      if (songInStore.id !== song.id) {
         ...
         if (songInStore.song_in !== "favorite" || actuallySongs.length !== favoriteSongsFiltered.length) {
   ...
   return (
      <>
            ...
            <div className="flex flex-row flex-wrap -mx-[8px]">
               {!!userPlaylists.length &&
                  ...
                  userPlaylists.map((playlist, index) => {
                     const active = isPlaying && !!songInStore?.song_in && songInStore.song_in.includes(playlist.id);
                     return (
               ...
               {initialLoading && PlaylistSkeleton}
               {
                  <div className={`${classes.playlistItem} mb-[25px]`}>
                     <Empty theme={theme} className="pt-[100%]" onClick={() => handleOpenModal("addPlaylist")} />
                  </div>
               }
            </div>
         </div>
         ...
         <div className="pb-[30px]">
            ...
            <div className="flex items-center justify-between">
               ...
               <label
                  className={`${theme.content_bg} ${
                     status === "uploading" || initialLoading ? "opacity-60 pointer-events-none" : ""
            ...
            {/* song list */}
            <div
               className={`flex gap-[8px] items-center h-[45px] no-scrollbar overflow-x-auto border-b border-${theme.alpha}`}
            >
               ...
               {isChecked && userSongs.length && (
                  <>
                     <Button
                        ...
                        variant={"outline"}
                        size={"small"}
                        className={`border-${theme.alpha} flex-shrink-0 ${theme.side_bar_bg} ml-[12px]`}
                     >
                     ...
                     >
                        <TrashIcon className="w-[20px] flex-shrink-0 mr-[4px]" />
                        Delete
                     </Button>

                     <Button onClick={resetCheckedList} className={`px-[5px] flex-shrink-0`}>
                  ...
                  >
                     {songTab === "mine" && !!songCount && (
                        <SongList
                           ...
                           activeExtend={songInStore.song_in === "user" || songInStore.song_in === "favorite"}
                           songs={userSongs}
                           tempSongs={tempSongs}
