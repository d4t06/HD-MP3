...
export default function PlaylistDetail() {
  ...
  const handleSelectAll = () => {
    if (selectedSongs.length < playlistSongs.length)
  ...
  const handleDeleteSongFromPlaylist = async (song: Song) => {
    try {
      const newPlaylistSongs = await deleteSongFromPlaylist(
        song,
        playlistSongs
      );
      if (newPlaylistSongs) {
  ...
  const classes = {
    ...
    infoTop:
      "flex justify-center items-center gap-[8px] md:flex-col md:items-start",
    songListContainer:
      "h-[50px] w-full mb-[10px] flex gap-[8px] max-[549px]:gap-[12px] items-center border-b",
    countSongText:
      "text-[14px]] font-semibold opacity-[.6] w-[90px] leading-[1]",
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="pb-[50px] mt-[30px]">
        <div
          className={`${classes.songListContainer} overflow-x-auto overflow-y-hidden border-${theme.alpha}`}
        >
