...
Props) {
  ...
  const { setUserSongs, userSongs, setAdminSongs, adminSongs } =
    useSongsStore();
  const { setActuallySongs, actuallySongs } = useActuallySongs();
  const { song: songInStore } = useSelector(selectAllSongStore);

  const [triggerUpdateActuallySongs, setTriggerUpdateActuallySongs] =
    useState<Song>();
  ...
  const handleInputChange = useCallback(
    async (e: ChangeEvent<HTMLInputElement>) => {
      ...
      const checkDuplicate = (songObject: Song) => {
        // console.log(
        //    "song list = ",
        //    processSongsList.map((s) => s.name)
        // );

        return (
          targetSongs.some(
            (song) =>
              song.singer === songObject.singer && song.name === songObject.name
          ) ||
      ...
      try {
        ...
        // case user
        if (!admin) {
          const actuallySongIds: string[] = processSongsList.map(
            (song) => song.id
          );
